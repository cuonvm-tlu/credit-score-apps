...
from app.core.l_diversity_adapter import OUTPUT_COLUMNS, anonymize_adult_spark_dataframe_l_diversity
...
def anonymize_cleaned_adult_l_diversity_and_upload(
    ...
    spark = get_spark_session(
        "preprocess-svc-anonymize-l-diversity",
        "spark-warehouse-preprocess-svc-ldiv",
    )
    ...
    records, _, _ = anonymize_adult_spark_dataframe_l_diversity(clean_df, l_value=l_value)

    anon_object_key = _build_anonymized_key(clean_object_key, l_value=l_value)
...
def _build_anonymized_key(clean_object_key: str, l_value: int) -> str:
    path = Path(clean_object_key)
    base = path.stem
    if base.endswith("_clean"):
        base = base[: -len("_clean")]
    anon_name = f"{base}_anon_l{l_value}.parquet"
